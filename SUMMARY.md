...
Sesi aktif: fitur saldo virtual **Tanpa dompet** dan allocation ledger dua arah selesai lokal, belum di-commit/deploy. `0012_add_wallet_balance_allocations.sql` menyimpan allocation immutable; saldo tersedia tetap canonical dari transaksi dan Tanpa dompet dihitung sebagai total tersedia dikurangi seluruh saldo wallet. UI menambah kartu virtual, dialog alokasi/reversal, histori, breakdown dashboard, selector wallet tabungan, dan grid wallet horizontal desktop. 142 Worker + 71 frontend = 213 test; typecheck/lint/format/build dan smoke API lokal hijau. `0012` baru diterapkan ke D1 lokal.

Snapshot lokal terbaru: **142 Worker test + 71 frontend test = 213**; typecheck, lint file perubahan, format, build, smoke API allocation, dan query D1 lokal hijau. Production version `a83b8138-8245-4d6e-b981-cfafb79f036d`; health 200 dan origin tetap diproteksi Access. Demo production version `fb719256-ec1a-40ca-96cb-4250dcd40771`.
...
- **Tanpa dompet**: bucket virtual per user; allocation dua arah tidak mengubah total saldo, saldo awal wallet baru menggunakan allocation, pasangan dapat melihat history read-only, dan setor/tarik tabungan dapat memilih wallet atau Tanpa dompet. Implementasi lokal selesai; deployment menunggu persetujuan.
...
- Migration `0012_add_wallet_balance_allocations.sql` baru diterapkan lokal; belum ke remote development/production.
