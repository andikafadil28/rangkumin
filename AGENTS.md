...
- Status: Phase 1–9, 11, dan 12 selesai; Phase 10 Google Sheets ditunda sebagai future update. Telegram runtime dihapus dan hanya menjadi kandidat add-on opsional. **Fitur Dompet (wallets) selesai diimplementasikan untuk lokal dan demo; belum di-deploy ke production.** Snapshot lokal terbaru: 133 Worker test + 70 frontend test (203); typecheck, lint file perubahan, format, dan build hijau. Fitur aktif sudah di-deploy ke production dan demo publik frontend-only aktif.

# CURRENT

Sesi aktif berikutnya: **fitur Dompet (wallets)** — tabel `wallets` baru + rebuild `transactions` (kolom `wallet_id`, `source_wallet_id`, `destination_wallet_id`, `reconciliation_status`, tipe `wallet_transfer`) via migration `0010_add_wallets.sql`. Backend (`src/routes/wallets.ts`, `src/services/wallets.ts`, `src/schemas/wallet.ts`), integrasi transaksi/tabungan/ownership, halaman `WalletsPage.tsx`, selector dompet di form transaksi, dan adapter demo selesai. Migration diterapkan ke DB lokal; seluruh test (133 Worker + 70 frontend) dan build hijau. Belum di-deploy ke remote development/production/demo, dan belum di-smoke-test di perangkat nyata. GitHub Release terbaru tetap `v1.0.0` pada commit `fe79904`. Production version `fe586f95-c7d1-421f-87a6-101bdc36e4b1`; health 200 dan origin diproteksi Cloudflare Access.
...
- Migration `0005_telegram_delivery.sql`, `0006_import_jobs.sql`, `0007_web_push.sql`, dan `0008_disable_telegram_channel.sql` sudah diterapkan ke lokal, remote development, dan production. `0008` menormalkan channel aktif ke Dashboard/Web Push. `0010_add_wallets.sql` (tabel `wallets` + rebuild `transactions`) sudah diterapkan ke DB lokal; belum diterapkan ke remote development/production.
...
- Demo publik: `https://demo.rangkumin.dikadevit.my.id`, Worker `rangkumin-demo`, version `9088ed20-46d5-46a3-91f4-3035a09f4548`. Scan Struk, Web Push, dan Import/Export dinonaktifkan; transaksi, tabungan, anggaran, pengingat, Trash, dan notifikasi disimulasikan di memori tab. Smoke root 200 dan `/api/me` hanya mengembalikan SPA HTML, sehingga tidak ada backend API pada origin demo.
- Test lokal terbaru: 133 Worker + 70 frontend = 203 (termasuk mode warna system/manual, serialisasi filter lanjutan, validasi rentang/sort, escaping LIKE, adapter demo, variasi respons receipt AI, label detail transaksi, perbandingan `summarizeTransactionTotals` + `getPreviousMonthRange`, insight dashboard, kategori expense-only, web-push, receiptImage, import/export, wallet service/route, mutasi tabungan dengan dompet, dan ownership transfer); typecheck, lint file perubahan, format, build, dan query D1 lokal hijau. Dashboard Insights, mode warna/filter sudah di-deploy tetapi belum di-smoke-test visual di perangkat nyata; Web Push belum di-smoke-test perangkat nyata; import/export belum diuji penuh dua arah di production; Dompet belum di-deploy/di-smoke-test.
