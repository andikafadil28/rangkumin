...
Rangkumin adalah aplikasi keuangan pasangan berbasis web untuk mencatat pemasukan, pengeluaran, saldo, tabungan, anggaran, dan pengingat. Core aplikasi bersifat open-source (MIT), dengan layanan setup/support komersial terpisah, dan berjalan secara serverless di Cloudflare.
...
- Status: Phase 1–9, 11, dan 12 selesai; Phase 10 Google Sheets ditunda sebagai future update. Telegram runtime dihapus dan hanya menjadi kandidat add-on opsional. **Fitur Dompet dan allocation saldo Tanpa dompet sudah di-deploy ke production, remote development, dan demo publik.** Snapshot lokal terbaru: 142 Worker test + 71 frontend test (213); typecheck, lint file perubahan, format, dan build hijau.

# CURRENT

Sesi aktif berikutnya: **fitur saldo Tanpa dompet + alokasi saldo ke dompet selesai dan sudah di-deploy.** Migration `0012_add_wallet_balance_allocations.sql` menambah ledger alokasi immutable dua arah (`to_wallet`/`to_unallocated`). Saldo tersedia tetap berasal dari ledger transaksi utama; saldo dompet menambahkan net allocation; saldo Tanpa dompet = saldo tersedia - seluruh saldo dompet (termasuk arsip), sehingga transaksi tanpa dompet tetap valid dan tidak ada double-count. GET `/api/wallets` menyertakan overview per user; POST `/api/wallets/allocations` memakai conditional INSERT atomik dan ownership guard; saldo awal wallet baru kini dicatat sebagai allocation. UI menambah kartu virtual Tanpa dompet, dialog Atur saldo dua arah, histori allocation, breakdown dashboard adaptif, selector wallet pada setor/tarik tabungan, serta grid desktop 3 kolom/2 tablet/1 mobile. Demo in-memory mengikuti kontrak yang sama. Verifikasi lokal: 142 Worker + 71 frontend = 213 test, typecheck/lint/format/build hijau; smoke API membuktikan saldo total tetap, over-allocation 409, reversal benar, dan histori pasangan read-only. Commit fitur `906d66e`; production version `19794943-e7c4-4bf9-b0b6-c2e9aa9a578d`; development version `cba8d366-cd5e-4d31-9318-0d44d9515fa4`; demo version `2003552d-921d-41a3-9208-da4f4fa41c0e`.

Keputusan dan hal penting:

- **Telegram tidak aktif.** Runtime bot (routes/services/test/scripts) dihapus; migration history, kolom, dan channel `notify_telegram` dipertahankan sebagai kompatibilitas historis. Telegram hanya kandidat add-on opsional di future update; kanal aktif saat ini Dashboard + Web Push.
- **Scan Struk**: `POST /api/receipt-scans` (multipart, cek MIME/magic/size/origin) memakai Workers AI `@cf/meta/llama-3.2-11b-vision-instruct`. Foto di-resize browser (`receiptImage.ts`, max 1800px, JPEG ≤2 MiB, EXIF ter-strip) dan tidak disimpan ke D1/R2/KV/Cache/IndexedDB. Hasil hanya **draft terkonfirmasi** (tidak auto-submit). Parser menormalisasi wrapper `draft`/`result`/`receipt`, direct draft, alias field umum, amount string, dan confidence persen; respons malformed dicoba ulang satu kali dengan prompt strict. Pricing/quota Workers AI dapat berubah dan wajib dicek dari dokumentasi resmi Cloudflare sebelum deployment baru.
...
- **Dashboard Insights**: `/api/summary` kini menyertakan `comparison: { period, combined } | null` berisi total bulan sebelumnya (`summarizeTransactionTotals`, satu query agregat; `getPreviousMonthRange` di `src/utils/date.ts`). Panel insight (`DashboardInsights` di `App.tsx`, helper `frontend/src/dashboardInsights.ts`) menampilkan headline perbandingan pengeluaran, kartu Pengeluaran/Pemasukan/Kategori terbesar, klasifikasi warna naik/turun, menghormati hide balance (`LiveMoney`) dan view mode Bersama/Saya. Kategori terbesar hanya menghitung tipe `expense` (koreksi dari sebelumnya yang ikut memasukkan income). `comparison` bernilai `null` bila hanya satu sisi rentang tanggal diberikan.
- **Dompet (wallets)**: halaman `WalletsPage.tsx` untuk membuat/mengelola dompet (cash/bank/e_wallet/other, grup, warna, saldo awal, dompet utama, arsip), transfer antar dompet (`wallet_transfer`, atomik, cek saldo via satu query agregat), detail mutasi dengan filter status rekonsiliasi (`unreconciled`/`reconciled`/`excluded`), dan quick-add transaksi ke dompet tertentu. Form transaksi kini punya selector dompet (hanya dompet aktif milik sendiri); income/expense/deposit/withdrawal dapat memakai `wallet_id` (opsional). Transfer antar dompet dan mutasi tabungan bersifat immutable di middleware ownership. Saldo dompet = `initial_balance` + jumlah income/withdrawal/transfer-in dikurangi expense/deposit/transfer-out pada transaksi aktif.
- **Saldo Tanpa dompet**: bucket virtual per user, bukan row `wallets`. Total saldo tersedia tetap canonical dari transaksi; saldo Tanpa dompet = total saldo tersedia - jumlah seluruh wallet termasuk arsip. Allocation dua arah dicatat di `wallet_balance_allocations`; mutasi hanya boleh oleh owner, tetapi pasangan dapat melihat historinya. Saldo awal wallet baru memakai allocation dan tidak menciptakan uang baru. Setor/tarik tabungan mendukung wallet atau Tanpa dompet; deposit tanpa wallet mengecek saldo bucket Tanpa dompet secara atomik.
- **GitHub Release**: release publik pertama `v1.0.0` dibuat manual dari commit `fe79904`; `v1.0.1` dari commit `e50494d` (fitur Dompet + mode warna, filter, insights); `v1.0.2` dari commit `906d66e` (saldo Tanpa dompet + allocation dua arah, grid wallet, selector wallet tabungan, dan fix CHECK warna); bukan draft atau prerelease. Release berikutnya tetap dibuat manual sesuai kebutuhan.
...
- Migration `0005_telegram_delivery.sql`, `0006_import_jobs.sql`, `0007_web_push.sql`, `0008_disable_telegram_channel.sql`, `0010_add_wallets.sql`, dan `0011_fix_wallet_color_check.sql` sudah diterapkan ke lokal, remote development, dan production. `0008` menormalkan channel aktif ke Dashboard/Web Push; `0010` (tabel `wallets` + rebuild `transactions`) untuk fitur Dompet; `0011` memperbaiki CHECK `color` di `wallets` yang memakai pattern GLOB `#[0-9A-Fa-f]…` (x6) yang ditolak D1 sebagai "LIKE or GLOB pattern too complex" sehingga create dompet berwarna selalu 500 — diganti predikat setara (`length=7` + `substr…='#'` + `substr(color,2,6) NOT GLOB '*[^0-9A-Fa-f]*'`) via rebuild tabel.
- Migration `0012_add_wallet_balance_allocations.sql` sudah diterapkan ke lokal, remote development, dan production.
...
- Demo publik: `https://demo.rangkumin.dikadevit.my.id`, Worker `rangkumin-demo`, version `2003552d-921d-41a3-9208-da4f4fa41c0e`. Scan Struk, Web Push, dan Import/Export dinonaktifkan; transaksi, tabungan, anggaran, pengingat, Wallet/Dompet, Trash, dan notifikasi disimulasikan di memori tab. Smoke root 200 dan `/api/me` hanya mengembalikan SPA HTML, sehingga tidak ada backend API pada origin demo.
- Test lokal terbaru: 142 Worker + 71 frontend = 213 (termasuk saldo Tanpa dompet, overview/allocation wallet dua arah, over-allocation, histori pasangan read-only, adapter demo, selector wallet tabungan, mode warna, filter, insights, web-push, receiptImage, import/export, dan ownership); typecheck, lint file perubahan, format, build, smoke API lokal, dan query D1 lokal hijau. Fitur allocation/grid wallet sudah di-deploy tetapi belum di-smoke-test visual perangkat nyata; Web Push belum di-smoke-test perangkat nyata; import/export belum diuji penuh dua arah di production.
...
- Google Sheets belum diimplementasikan dan hanya kandidat future update; jika kelak dibuat, D1 tetap source of truth.
...
- Google Sheets bersifat roadmap opsional tanpa timeline; jika kelak dibuat, kegagalan sinkronisasi tidak boleh menggagalkan transaksi D1.
...
- [x] Lindungi semua endpoint aplikasi selain health; tidak ada endpoint webhook publik.
...
## Phase 9 - Penggantian Telegram

- [x] Hapus runtime Telegram Bot dan ganti kanal aktif dengan Web Push. Migration history/kolom lama dipertahankan untuk kompatibilitas; integrasi Telegram hanya kandidat add-on future update.

## Phase 10 - Google Sheets (Deferred)

- [ ] Future update opsional tanpa timeline: adapter, scheduled/manual sync, retry, dan status; D1 wajib tetap authoritative.
...
- [x] Selesaikan README dan setup guide bilingual serta dua PDF terpisah.
- [x] Tambahkan MIT `LICENSE`, `SECURITY.md`, `CONTRIBUTING.md`, `CODE_OF_CONDUCT.md`, dan penjelasan layanan komersial.
- [x] Tambahkan `.github/FUNDING.yml` menuju Buy Me a Coffee.
...
- Jangan pernah commit private key, token, identifier/email pengguna, Service Account JSON, spreadsheet ID production, foto struk, atau transaksi nyata.
...
- Jika webhook Telegram opsional kelak dibuat, endpoint publik wajib memakai secret, allowlist, rate limit, dan idempotency; versi aktif tidak memiliki webhook publik.
