...
- **GitHub Release**: release publik pertama `v1.0.0` dibuat manual dari commit `fe79904`; `v1.0.1` dari commit `e50494d` (fitur Dompet + mode warna, filter, insights); bukan draft atau prerelease. Release berikutnya tetap dibuat manual sesuai kebutuhan.
...
- Migration `0012_add_wallet_balance_allocations.sql` baru diterapkan ke lokal; belum ke remote development/production.
...
- Demo publik: `https://demo.rangkumin.dikadevit.my.id`, Worker `rangkumin-demo`, version `fb719256-ec1a-40ca-96cb-4250dcd40771`. Scan Struk, Web Push, dan Import/Export dinonaktifkan; transaksi, tabungan, anggaran, pengingat, Wallet/Dompet, Trash, dan notifikasi disimulasikan di memori tab. Smoke root 200 dan `/api/me` hanya mengembalikan SPA HTML, sehingga tidak ada backend API pada origin demo.
- Test lokal terbaru: 142 Worker + 71 frontend = 213 (termasuk saldo Tanpa dompet, overview/allocation wallet dua arah, over-allocation, histori pasangan read-only, adapter demo, selector wallet tabungan, mode warna, filter, insights, web-push, receiptImage, import/export, dan ownership); typecheck, lint file perubahan, format, build, smoke API lokal, dan query D1 lokal hijau. Fitur allocation/grid wallet belum di-deploy atau di-smoke-test visual perangkat nyata; Web Push belum di-smoke-test perangkat nyata; import/export belum diuji penuh dua arah di production.
